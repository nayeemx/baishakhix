...
const Upload = () => {
  ...
  // PGSQL to Excel conversion
  const pgsqlToExcel = async (sqlText) => {
    const jsonData = await pgsqlToJson(sqlText);
    if (!jsonData) return null;
    // Create a workbook with each table as a sheet
    ...
    if (conversionType === "csv") {
      ...
      runProgressAnimation(2200, setConvertProgress, async () => {
        ...
        if (!jsonData) {
          ...
          setShowResult(false);
          setJsonOutput(null);
          toast.error("CSV conversion failed."); // Ensure error toast
          return;
        }
        setJsonOutput(jsonData);
        setShowResult(true);
        setConverting(false);
        toast.success("CSV converted to JSON!"); // Show success toast

        // Store meta data in uploads collection only if not already present for this fileName
        try {
          if (user && user.uid && uploadedFileName) {
            const uploadsRef = collection(firestore, "uploads");
            const q = query(uploadsRef, where("fileName", "==", uploadedFileName));
            ...
            // else: do not add duplicate meta data for the same file
          }
        } catch (err) {
          toast.error(`Failed to save conversion meta: ${err.message}`);
        }
      });
      ...
      runProgressAnimation(2200, setConvertProgress, async () => {
        ...
        if (!jsonData) {
          ...
          setShowResult(false);
          setJsonOutput(null);
          toast.error("PGSQL conversion failed."); // Ensure error toast
          return;
        }
        setJsonOutput(jsonData);
        setShowResult(true);
        setConverting(false);
        toast.success("PGSQL converted to JSON!"); // Show success toast
        // Optionally: Save meta to Firestore for PGSQL as well
      });
