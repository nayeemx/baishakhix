...
const BarCode = () => {
  ...
  const handleClear = async () => {
    ...
    try {
      ...
      console.log("Products to update (Firestore IDs):", productsToUpdate.map(p => p.firestoreId));

      const batch = [];
      for (const product of productsToUpdate) {
        const productRef = doc(firestore, 'products', product.firestoreId);
        batch.push(updateDoc(productRef, { is_labeled: "t" }));
      }

      await Promise.all(batch);
      console.log(`Successfully updated ${productsToUpdate.length} documents`);

      // Refresh products state
      const updatedSnap = await getDocs(collection(firestore, 'products'));
      const updatedProducts = updatedSnap.docs
        .map(doc => ({ firestoreId: doc.id, ...doc.data() }))
        .filter(p => p.barcode && p.is_labeled !== undefined && p.quantity !== undefined);
  ...
  return (
    <div className="p-4">
      ...
      <div ref={printAreaRef} className="barcode-print-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-7 print:-ml-[1vw] print:grid-cols-6 print:gap-2 gap-4">
