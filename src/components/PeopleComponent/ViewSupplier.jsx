...
const ViewSupplier = ({ supplier }) => {
  ...
  // Define the handler before using it in JSX
  ...
  useEffect(() => {
    ...
    getDocs(
      ...
      // Fetch supplier_transaction for this supplier only if bill numbers exist
  ...
  // Filter products by bill_number
  ...
  // Calculate bill summary logic
  // 1. Get unique bill_numbers for this supplier (filtered if searching)
  const billNumbers = Array.from(
    new Set(filteredProducts.map((p) => p.bill_number || ""))
  ).filter((b) => b);

  // 2. For each bill_number, pick the first product's deal_amount and paid_amount
  const billSummaries = billNumbers.map((bill_number) => {
    const billProducts = filteredProducts.filter(
      (p) => p.bill_number === bill_number
    );
    // Pick the first non-empty deal_amount and paid_amount for this bill_number
    const dealAmount = billProducts.find(
      (p) => p.deal_amount && !isNaN(Number(p.deal_amount))
    );
    const paidAmount = billProducts.find(
      (p) => p.paid_amount && !isNaN(Number(p.paid_amount))
    );
    return {
      bill_number,
      deal_amount: dealAmount ? Number(dealAmount.deal_amount) : 0,
      paid_amount: paidAmount ? Number(paidAmount.paid_amount) : 0,
    };
  });

  // 3. Sum up deal_amount and paid_amount for all unique bill_numbers
  ...
  // 5. If searching by bill_number, show only that bill_number's summary
  let showBillNumber = "";
  ...
  // Calculate sum of Quantity, Unit Price, Total Price for filtered products
  ...
  // Count of unique bill_number in filtered products
  const uniqueBillNumbers = Array.from(
    new Set(filteredProducts.map((p) => p.bill_number || ""))
  ).filter((b) => b);
  const billNumberCount = uniqueBillNumbers.length;

  // Pass bill numbers and products to SupplierTransaction
  ...
  const handleEditSave = async (e) => {
    ...
    try {
      ...
      setEditOpen(false);
      // Optionally trigger a refresh if you want to reload supplier info
      // setRefreshFlag(f => f + 1);
    } finally {
      setEditSaving(false);
    }
