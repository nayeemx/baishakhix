...
const EditProductModal = ({
  ...
  useEffect(() => {
    setForm(data || {});
    const imgs = typeof data?.image === 'string'
      ? data.image.split(',').map(i => i.trim()).filter(Boolean)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const duplicate = snapshot.docs.find(docSnap => docSnap.id !== data.id);
      if (duplicate) {
      ...
      await updateDoc(doc(firestore, 'products', data.id), saveForm);
      setOpen(false);
